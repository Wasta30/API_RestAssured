 ...
 */
    ...
    public void postMethod1() {
        ...
        String requestJsonBody = "{\n" +
                ...
                "      \"phone\":8877445596\n" +
                "   }";
// save json insdie string and
// we have two accept content type , one is what we are asking from api other one is what we are sending to api
        ...
        Response response = given().accept(ContentType.JSON).and()//what we are asking from api which is JSON response
                ...
                .body(requestJsonBody)// we are adding body to our package i mean to our api
...
// save json insdie string and
// we have two accept content type , one is what we are asking from api other one is what we are sending to api
// from java to json it will automatically convert by restassured librarby
        Response response = given().accept(ContentType.JSON).and()//what we are asking from api which is JSON response
                .contentType(ContentType.JSON) //what we are sending to api, which is JSON also
                .body(requestJsonMap).log().all()// we are adding body to our package i mean to our api
                .when().post("/api/spartans");
    ...
    public void postMethod3() {
        ...
        Response response = given().accept(ContentType.JSON).and()//what we are asking from api which is JSON response
                .contentType(ContentType.JSON) //what we are sending to api, which is JSON also
                .body(spartan).log().all()// we are adding body to our package i mean to our api
                .when().post("/api/spartans");
    ...
    public void postMethod4() {
        // when u sned post req how u make usre it send successfully , u got 201 and u got id value and use it as separate get reuquest
        // we post somehting
